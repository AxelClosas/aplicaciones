...
def run():
    ...
    for item in fecha_entrega_2023:
        if item["Producto origen"] in vacunas_distribuidas.keys():
            vacunas_distribuidas[item["Producto origen"]] += item["Cantidad origen"]
        else:
            vacunas_distribuidas[item["Producto origen"]] = item["Cantidad origen"]

    for key, value in vacunas_distribuidas.items():
        print(key, value)
